...
export default function ManageBookings() {
  ...
  const [mode, setMode] = useState<
    "view" | "reschedule" | "cancelled_success" | "rescheduled_success"
  >("view");
  ...
  const [availableSlots, setAvailableSlots] = useState<
    { time: string; availableArtists: any[] }[]
  >([]);
  ...
  const handlePhoneLookup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (data.length === 0) {
          setLookupError(
            "No booking records matching this mobile number found in Colombo databases.",
          );
        }
      }
    } catch (err) {
  ...
  const handleRescheduleBooking = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        setActionError(
          data.error || "The selected slot is busy. Please pick another.",
        );
        return;
      }
    ...
    <div className="max-w-2xl mx-auto py-4 space-y-10 animate-fadeIn">
      <div className="text-center max-w-lg mx-auto space-y-1">
        ...
        <p className="text-stone-500 text-sm font-light">
          Lookup, reschedule, or cancel your upcoming beauty slots instantly
          without login.
        </p>
      </div>
      ...
      <section className="bg-white rounded-3xl p-6 border border-[#C5A059]/15 shadow-sm space-y-4">
        <h3 className="font-serif font-bold text-[#1A1A1A] text-sm flex items-center">
          <Phone className="w-4 h-4 text-[#C5A059] mr-2 shrink-0" /> Search by
          Mobile Number
        </h3>
            ...
            {lookups.map((bk) => {
              ...
              return (
                <div
                  ...
                  className="bg-stone-50/50 border border-stone-200/65 rounded-xl p-4 flex flex-col sm:flex-row justify-between sm:items-center gap-4"
                >
        ...
        <section className="bg-[#FAF8F5] rounded-2xl p-6 border border-[#C5A059]/20 shadow-md animate-fadeIn space-y-4">
          <div className="flex justify-between items-center">
            <h4 className="font-serif font-bold text-stone-905 text-sm flex items-center uppercase tracking-wide">
              <Key className="w-4 h-4 text-[#C5A059] mr-1.5" /> Security Access
              Verification
            </h4>
          ...
          <p className="text-xs text-stone-500">
            Provide the 4-digit security PIN sent on confirmation screen to
            self-manage this booking:
          </p>
        ...
        <section className="bg-white border border-[#C5A059]/20 shadow-md rounded-3xl p-6 md:p-8 space-y-6 animate-fadeIn">
          <div className="border-b border-[#C5A059]/10 pb-4 text-center">
            <span className="inline-block text-[10px] font-mono font-bold bg-[#C5A059]/10 text-[#C5A059] px-3 py-1 rounded-full uppercase">
              Management Portal Authorized &bull;{" "}
              {authedBooking.bookingReference}
            </span>
            <h3 className="font-serif text-xl font-bold text-stone-905 mt-1">
              Hello, {authedBooking.customerName}!
            </h3>
          </div>
          ...
          <div className="space-y-3 bg-stone-50 p-4 rounded-xl border border-stone-150 text-xs text-stone-700">
            ...
            <p className="flex justify-between">
              ...
              <span className="text-[#C5A059] font-mono font-bold">
                {authedBooking.startTime} - {authedBooking.endTime}
              </span>
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            >
              ...
              <span>
                {submittingAction ? "Cancelling..." : "Cancel Appointment"}
              </span>
            </button>
          </div>
            ...
            <div>
              ...
              {slotsLoading ? (
                ...
                <p className="text-xs text-[#C5A059] italic">
                  No available times on this date. Please pick a different date.
                </p>
              ) : (
      ...
      {mode === "cancelled_success" && authedBooking && (
        <section className="bg-emerald-50 rounded-2xl p-6 border border-emerald-250 text-center space-y-4 animate-fadeIn">
          ...
          <h4 className="font-serif font-bold text-emerald-900 text-lg leading-tight">
            Session Cancelled Successfully
          </h4>
          <p className="text-emerald-700 text-xs leading-relaxed max-w-sm mx-auto font-light">
            Your beauty appointment under reference{" "}
            {authedBooking?.bookingReference ?? "N/A"} has been cancelled
            correctly in Colombo registers. Feel free to book again at any time!
          </p>
          <button
            onClick={() => {
              setAuthedBooking(null);
      ...
      {mode === "rescheduled_success" && authedBooking && (
        <section className="bg-emerald-50 rounded-2xl p-6 border border-emerald-250 text-center space-y-4 animate-fadeIn">
          ...
          <p className="text-emerald-700 text-xs leading-relaxed max-w-sm mx-auto font-light">
            Your beauty appointment is rescheduled to the new secure slot:{" "}
